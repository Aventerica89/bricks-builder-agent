...
function storeApiKey(params: {
  ...
  vault?: string
  tags?: string[]
}): { id: string; title: string; vault: string } {
...
function listApiKeys(params: {
  provider?: string
  vault?: string
}): Array<{ id: string; title: string; vault: string; tags: string[] }> {
  ...
  return items.map((item: { id: string; title: string; vault?: { name: string }; tags?: string[] }) => ({
    id: item.id,
    title: item.title,
...
function searchItems(params: {
  ...
  tags?: string[]
  vault?: string
}): Array<{ id: string; title: string; vault: string }> {
  ...
  if (query) {
    ...
    items = items.filter((item: { title: string }) =>
      item.title.toLowerCase().includes(queryLower)
    )
  }

  return items.map((item: { id: string; title: string; vault?: { name: string } }) => ({
    id: item.id,
    title: item.title,
    vault: item.vault?.name || DEFAULT_VAULT
  }))
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case 'store_api_key':
        result = storeApiKey(args as Parameters<typeof storeApiKey>[0])
        break

      case 'list_api_keys':
        result = listApiKeys(args as Parameters<typeof listApiKeys>[0])
        break

      case 'get_api_key':
        result = getApiKey(args as Parameters<typeof getApiKey>[0])
        break

      case 'add_token_to_existing':
        result = addTokenToExisting(args as Parameters<typeof addTokenToExisting>[0])
        break
